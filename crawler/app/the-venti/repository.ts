import { inArray } from "drizzle-orm";

import { cafeCategories, cafes, menus, menuVariants } from "@/drizzle/schema";
import { db } from "@/lib/db";
import {
  THE_VENTI_CAFE,
  THE_VENTI_CATEGORIES,
  THE_VENTI_ROOT_CATEGORIES,
} from "./cafe";
import { type TheVentiCrawlResult } from "./types";

const THE_VENTI_CHILD_CATEGORIES = THE_VENTI_CATEGORIES.map((category) => ({
  id: category.id,
  cafeId: category.cafeId,
  parentId: category.parentId,
  name: category.name,
  sortOrder: category.sortOrder,
  isVisible: category.isVisible,
}));

export async function replaceTheVentiMenus({
  menus: newMenus,
  variants,
}: TheVentiCrawlResult): Promise<void> {
  const categoryIds = THE_VENTI_CHILD_CATEGORIES.map(
    (category) => category.id,
  );

  await db.transaction(async (tx) => {
    await tx
      .insert(cafes)
      .values(THE_VENTI_CAFE)
      .onConflictDoUpdate({
        target: cafes.id,
        set: {
          name: THE_VENTI_CAFE.name,
          description: THE_VENTI_CAFE.description,
          isActive: true,
          updatedAt: new Date(),
        },
      });

    for (const category of [
      ...THE_VENTI_ROOT_CATEGORIES,
      ...THE_VENTI_CHILD_CATEGORIES,
    ]) {
      await tx
        .insert(cafeCategories)
        .values(category)
        .onConflictDoUpdate({
          target: cafeCategories.id,
          set: {
            cafeId: category.cafeId,
            parentId: category.parentId,
            name: category.name,
            sortOrder: category.sortOrder,
            isVisible: category.isVisible,
            updatedAt: new Date(),
          },
        });
    }

    await tx.delete(menus).where(inArray(menus.categoryId, categoryIds));
    await tx.insert(menus).values(newMenus);
    await tx.insert(menuVariants).values(variants);
  });
}
